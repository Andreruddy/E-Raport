...
class siswaController extends Controller
{
    ...
    {
        if ($request->has('cari')){
            $siswa = siswa::where('nama_siswa', 'LIKE', '%'.$request->cari.'%')->paginate();
        }else{
    ...
    public function update(Request $request, $id)
    {
        ...
        // ]);
        $this->_validation($request);
        siswa::where('id', $id)->update(['nis' => $request->nis, 'nama_siswa' => $request->nama_siswa, 'ttl' => $request->ttl, 'tgl_lahir' => $request->tgl_lahir, 'agama' => $request->agama, 'alamat' => $request->alamat, 'update_by'=>auth()->user()->id]);
